...
ps aux 2>/dev/null | head -n 6 || ps | head -n 6
...
> `read -p` only displays its prompt when input is a terminal, so when running non-interactively
> the prompts won't appear, but the input is still read correctly.
